    ...
    return (
        ...
        <div className="grid grid-cols-6 justify-items-center gap-10 lg:px-20 container mx-auto pb-20">
            ...
            list3={['【2回目(30,000円)：開通月を1カ月目として12カ月目】※新規の方のみ',<br key='list3'/>,'開通月を1カ月目として12カ月目が適用月となり、適用月の月初2日～45日間以内にBIGLOBEのマイページからの受け取り手続きが必要となります。適用月にBIGLOBEより、BIGLOBEメール、BIGLOBEにご登録の連絡先メールアドレス宛にメールが送信されます。受け取り手続きが完了後、数日以内にご指定の口座にキャッシュバックされます。※期間内に受け取り手続きされない場合は特典をお受け取りいただくことが出来ません。※受け取り期間前に退会、コース変更、移転をされたお客さまはキャンペーン適用外となります。']}
             ...
             <Notes 
             title={'その他注意事項'} 
             list1={'本キャンペーンは対象のサービスを8カ月以上継続して利用していただく事が条件となります。上記期間内の解約（お引越し等による解約や引越し先エリア外継続利用不可も含む）や料金不払いによる強制解約となった場合、本キャンペーンの違約金としてキャッシュバック金額を全額ご請求させていただきます。お引越し先で継続利用をしていただければ本キャンペーン違約金の対象外となります。'} 
             list2={'本キャンペーンの権利を第三者へ譲渡することはできません。譲渡が発覚した場合、本キャンペーンは無効とさせていただきます。'} 
             list3={'工事費実質無料を選択され、工事費割賦期間、継続利用期間内に解約や料金不払いによる強制解約となった場合、割引は停止となり、残債はお支払いいただきます。'} 
             list4={'当サイト経由以外でのお申込みは本キャンペーンの対象外となります。'} 
             list5={'本キャンペーンの適用条件は予告無く変更となる場合があります。。'} 
             list6={'本キャンペーンにお申込みをされた場合は、適用条件のすべての条件に同意いただいたものとみなされます。予めご了承ください。'} 
             list7={'本キャンペーンにお申込みをされた場合は、適用条件のすべての条件に同意いただいたものとみなされます。予めご了承ください。'} 
             list8={'サイト内に記載の当社キャッシュバック還元率は、設備の提供状況やお客様都合によるキャンセルなどで、ビッグローブ光を開通されなかった場合を除きます。'} 
             list9={'キャンペーン提供主体：株式会社リスターツ'} 
             />
        ...
        <div className="grid grid-cols-6 justify-items-center gap-10 lg:px-20  container mx-auto pb-20">
            ...
            <Notes 
             title={'注意事項'} 
             list1={' 機器解約時には、返送手数料（解約手数料）として1,430円（機器返却送料含む）がかかりますが特典により無料となります。。'} 
             list2={'解約から1カ月を過ぎても機器が返却されない場合、端末代金相当額としてAterm WG1200HS3の場合は 6,600円が請求されます。接続機器（無線LAN付き）は概ね以下日程で発送されます'} 
             list3={'－回線新規の場合 概ね開通工事の前※開通工事後となる場合もあります（不達の場合、工事予定日を前倒し変更した場合など）'} 
             list4={'－転用の場合 NTT東日本エリアの場合、概ねビッグローブ光への切り替え前 NTT西日本エリアの場合、概ねビッグローブ光への切り替え後7日以内'} 
             list5={'ビッグローブ光と同時申し込みの場合、接続機器（無線LAN付き）の契約開始日は、「機器の着荷をビッグローブが認識した日」と「ビッグローブ光の契約開始を認識した日」のうち遅い方の日となります。'} 
             list6={'本キャンペーンにお申込みをされた場合は、適用条件のすべての条件に同意いただいたものとみなされます。予めご了承ください。'} 
             list7={'接続機器（無線LAN付き）の着荷がビッグローブ光の契約開始後となった場合は、着荷日を起点として接続機器（無線LAN付き）の月額料金と値引きが開始します。'} 
             list8={'ビッグローブ光をキャンセルした場合、接続機器（無線LAN付き）のお申込みもキャンセルとなります。接続機器（無線LAN付き）の着荷後、ビッグローブ光をキャンセルされた場合、返却キットがお客様に送付されます。返却に関わる代金は不要です。'} 
             list9={'接続機器（無線LAN付き）を 使った高速接続サービス（IPv6接続（IPoE方式））をご利用の場合、「固定IPアドレスオプション／ダイナミックDNSサービス」「一部通信型ゲームなど特定 ポートを使用するサービスまたは複数のユーザでIPアドレスを共有すると利用できないサービス」 「外部サーバへの公開」が利用できなくなります。機器は予告なく変更となる場合があります。'} 
             />
        ...
        <div className="grid grid-cols-6 justify-items-center gap-10 lg:px-20  container mx-auto pb-20">
            ...
            <Notes 
             id={'au-notes'}
             title={'割引適用についての注意事項'} 
             list1={'ご契約はご自宅のインターネットサービス1回線につき、au携帯電話(タブレット含む)合計10回線までです。「auスマートバリュー」をご契約の回線が「家族割プラス」グループまたはUQ mobileの「自宅セット割」グループにも加入している場合、各グループにご加入のau/UQ携帯電話も回線数のカウントに含まれます。ご自宅のインターネットサービスとau携帯電話(タブレット含む)のご契約者が同一姓・同一住所の場合、お申し込みできます。50歳以上のご家族の場合は、対象のご自宅のインターネットサービス(「ネット＋電話」、「ネット＋テレビ」 、「テレビ＋電話」 )にご契約している住所と別住所でもお申し込みできます。別姓やau携帯電話(タブレット含む)が累計5回線以上の場合はご家族・同一住所であることの証明書が必要です。auスマートバリューのお申し込み翌月以降から割引適用となります。'} 
             list2={'スマホセット割ご加入の方がauスマートバリューを申し込みされた場合、自動的にauスマートバリューの割引に切り替わります。ただしauスマートバリュー適用前にスマホセット割を廃止した場合、前月末をもってスマホセット割は終了となり、auスマートバリューは翌月から割引が適用となります。auスマートバリューご加入から2年間の割引(2,200円/月(税込)、1,551円/月(税込)、1,027円/月(税込))適用は、対象のau携帯電話(タブレット含む)1回線につき、auスマートバリューの割引を初めて適用した月を1ヶ月目としてカウントし、最大24ヶ月の割引をもって終了します。3年目以降は割引額が変更となります。'} 
             list3={'割引対象合計額が割引金額を下回る場合は、割引対象合計額を上限として割引します。割引額は、月末時点で加入しているau携帯電話の料金プランにより判定します。月末時点で条件を満たさない月は、割引は適用されません。au携帯電話(タブレット含む)を解約 (一時休止、譲渡) をされた場合、割引の適用は解約 (一時休止) 月の当月、または譲渡月の前月をもって終了します。'} 
             list4={'ご自宅のインターネットサービスを「利用中」または「手続き中」の場合も、割引の適用となります。ご自宅のインターネットサービスを解約などをされた場合、割引の適用は解約月の前月をもって終了します。'} 
             list5={'auスマートバリューお申し込み後、ご自宅のインターネットサービス (「ネット+電話」、「ネット＋テレビ」、「テレビ＋電話」)が6ヶ月経過後も開通されていない場合は、順次、割引を一時停止します。開通後、再び割引を再開します。ご自宅のインターネットサービスを提供会社側都合で取消された場合は、取消月の翌月から3ヶ月目まで割引を適用します。ただし、開通前にお客さま都合により取消された場合は、受付の前月利用分をもって割引を終了し、それまでの割引額をau携帯電話(タブレット含む)側のご利用料金に合算し請求します。'} 
             list6={'別途、機種代金、通話・通信料、契約にかかる費用、ユニバーサルサービス料、電話リレーサービス料などがかかります。'} 
             list7={'「ケーブルプラス電話　auケータイセット割」等、KDDIが実施する他の施策とは併用できない場合があります。'} 
             list8={'ビッグローブ・KDDIが実施する他の施策とは併用できない場合があります。'} 
             list9={'法人向けビッグローブ光は割引対象外となります。'} 
             />
        ...
        <div className="grid grid-cols-6 justify-items-center gap-10 lg:px-20  container mx-auto pb-20">
           ...
           <div className="block lg:hidden">
            ...
            <Notes
             id={'uq-notes'}
             title={'割引適用についての注意事項'} 
             list1={'対象サービス1契約につき、UQ mobile回線合計10回線(BIGLOBE WiMAX +5Gの場合、サービス1回線につき、UQ mobile合計9回線)までとなります。「くりこしプラン +5G」でご家族がau回線をお持ちの場合、割引となるUQ mobile回線は、ご家族のau回線と合わせて10回線までとなります(au回線は⾃宅セット割の割引対象外です)。対象サービスとUQ mobileのご契約者が同一姓・同一住所の場合、お申し込みできます。ただし、50歳以上のご家族の場合は対象サービスにご契約している住所と別住所でもお申し込みできます。別姓やUQ mobile回線が累計5回線以上の場合はご家族・同一住所であることの証明書が必要です(別住所で50歳以上はご家族の証明書のみ)。'} 
             list2={'自宅セット割 お申し込み翌月以降、条件を満たした月のご利用分から割引を適用します。「でんきコース」と「インターネットコース」は重複して適用されず、「インターネットコース」を適用します。'} 
             list3={'UQ mobileの解約をした場合、お手続きの当月分をもって割引の適用を終了します。UQ mobileの月額基本料が日割となる場合は本割引も日割を適用します。対象サービスが開通前の場合も割引の適用となりますが、適用開始から6ヵ月経過後も開通されていない場合は、順次、割引を終了します。ただし、お申し込み条件外などの理由により開通できないことを当社が確認した場合は、6ヵ月以内でも割引を終了する場合があります。お客さま都合により対象サービスのお申し込みを取消された場合は、割引を終了し、それまでの割引額をUQ mobileのご利用料金に合算し請求する場合があります。'} 
             list4={'割引額は、月末時点で加入しているUQ mobileの料金プランにより判定します。月末時点で条件を満たさない月は、割引は適用されません。対象サービスを解約された場合、割引の適用は解約月の前月をもって終了します。'} 
             list5={'「インターネットコース」適用中のお客さまがauへ番号移行し、「auスマートバリュー」の適用条件を満たした場合、番号移行月の翌月より「auスマートバリュー」が自動で適用されます。 auで「auスマートバリュー」適用中のお客さまがUQ mobileに番号移行し、「インターネットコース」の適用条件を満たした場合、番号移行の翌月より「インターネットコース」が自動で適用されます。'} 
             list6={'新規契約または機種変更と同時にお申し込みの場合、料金プランは加入日または変更日当日から適用となります。プラン変更の場合は、翌月から適用となります。'} 
             list7={'auおよびpovo2.0への番号移行の場合、基本料および当社所定の割引サービス、オプションサービス料金等は番号移行の完了日の前日までの日割りとなりますが、当面の間、全額を一旦ご請求した上で、番号移行の翌月以降に差額を調整します。ただし、沖縄セルラーへのMNPは対象外です。             '} 
             list8={'ビッグローブ・KDDIが実施する他の施策とは併用できない場合があります。'} 
             list9={'別途、機種代金、通話・通信料、契約にかかる費用、ユニバーサルサービス料、電話リレーサービス料などがかかります。'} 
             />
